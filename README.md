...
\end{align}
```
From ```FEniCS``` Leveragin the ```GMRES``` algorithm and the ```ilu``` preconditioner $(2),(3)$ can be assembled and solved. The outcome I got are in ```/media```, whereas one extract is:
